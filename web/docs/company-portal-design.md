...
**Фаза 1 - MVP (компания может выложить вакансию и получить отклики):**
компании + членство (owner создаёт, ролей две: owner/recruiter через
инвайты), CRUD вакансий с премодерацией, отклик кандидата с резюме,
список откликов со сменой статуса, email-уведомления (синхронно) про
отклик и смену статуса, страница модерации, лендинг `/for-companies`,
обновление consent.

**Фаза 2 - роли и уведомления полностью:** роль admin + матрица прав,
in-app колокольчик + notification_prefs, email-outbox с дайджестом,
напоминания про невскрытые отклики, trusted-автопубликация,
`/office/applications` у кандидата (в MVP - хотя бы список без деталей).

**Фаза 3 - рост:** публичные страницы компаний `/companies/[slug]` (SEO),
статистика вакансии (просмотры/отклики/конверсия), шаблоны ответов
рекрутера, поиск по откликам, передача владения компанией.
